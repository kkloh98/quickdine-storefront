    ...
    <v-card>
          ...
          <v-row>
            <v-col cols="6" sm="6" md="4" class="py-0">
              <v-text-field
                ...
                name="first_name"
              ></v-text-field>
              <span v-if="errors.has('first_name')" style="font-size: 12px;" class="red--text"
            ...
            <v-col cols="12" sm="6" md="4" class="py-0">
              ...
              ></v-text-field>
              <span v-if="errors.has('email')" style="font-size: 12px;" class="red--text">* Email is required</span>
            </v-col>
            <v-col v-if="form == 'register'" cols="12" class="py-0">
              <v-text-field v-model="customer.phone" color="green darken-2" label="Phone" type="phone"></v-text-field>
            </v-col>
            <v-col cols="12" class="py-0">
              <v-text-field
                ...
                v-validate="'required'"
              ></v-text-field>
              <span v-if="errors.has('password')" style="font-size: 12px;" class="red--text"
            ...
            <v-col v-if="form == 'register'" cols="12" class="py-0">
              ...
              ></v-text-field>
              <p style="font-size: 12px;" v-if="errors.has('confirm_password')" class="red--text">
      ...
      <v-card-actions>
        <v-row justify="center" class="ma-2">
          <v-btn v-if="form == 'register'" @click="register()" dark color="green darken-2" width="100%">SIGN UP</v-btn>
          <v-btn v-else @click="login()" dark color="green darken-2" width="100%">SIGN IN</v-btn>
        </v-row>
      </v-card-actions>
      <v-card-subtitle class="text-center blue--text" style="font-size: 12px;" @click="changeForm()">{{
...
export default {
  ...
  methods: {
    ...
    async register() {
      ...
      if (res.success) {
        Cookies.set('token', { value: res.token, store_id: this.$route.params.store_id })
        this.clearFields()
        this.$emit('input')
      }
    },
